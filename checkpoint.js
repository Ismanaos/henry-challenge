...
function menorMayor(numeros) {
  ...
  maxMin.push(Math.min.apply(null, numeros));
  maxMin.push(Math.max.apply(null, numeros));
...
function stringMasLarga(strings) {
  ...
  let totalStrings  = [];
    for(let palabra of strings) {
      totalStrings.push(palabra.length);
    }
    
    let max = Math.max.apply(null, totalStrings);
...
function buscarAmigo(amigos, nombre) {
  ...
  for (let i = 0; i < amigos.length; i++) {
    ...
    var valor = Object.values(element);
    for (let j = 0; j < valor.length; j++) {
      const element = valor[j];
      if (valor[j] === nombre) {
        return amigos[i]
      }
    }
...
function sumArray(array, n) {
  ...
  for(let i = 0; i < array.length; i++){
    ...
    for(let j = 0; j < array.length; j++){
      const elementj = array[j +1];
      if(array[j] != array[i]){
        if(element + elementj == n){
            if((element + elementj) % n == 0){
              return true
            };
        };
      };
...
function pluck(array, propiedad) {
  ...
  // Tu código acá:
  var productosFiltrados = array.map(function(propiedades){
...
function crearClasePersona() {
  class Persona {
    ...
    addFriend(nombre, edad) {
      ...
      this.nombre1 = nombre;
      this.edad1 = edad;
      this.amigos.push({nombre: this.nombre1,edad: this.edad1});
    }
    ...
    getFriends() {
      ...
      // Tu código aca:
      var names = this.amigos.map(function(nombres){
    ...
    getPromedioEdad() {
      ...
      // Tu código aca:
      var yearsOld = this.amigos.filter(function(edades){return edades.edad})
      .map(function(edades){return edades.edad})
      .reduce(function(contador, promedio){
        return (contador + promedio) / 2
      })
      return yearsOld;
    }
  };
